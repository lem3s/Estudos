...
func main() {
	...
	ht := NewHashTable(
		2000033,
	) // Tamanho primo para reduzir colisões, ajustado para ~2 milhões de slots

	// Carregar números do arquivo na tabela hash
	err := loadNumbers("RandomNumbers.txt", ht)
	if err != nil {
		fmt.Println("Erro ao carregar número:", err)
		return
	}
	...
	for i := 0; i < 100; i++ {
		...
		if index, found := ht.Get(num); found {
			fmt.Printf("Número %d encontrado no index %d\n", num, index)
		} else {
			fmt.Printf("Número %d não encontrado\n", num)
		}
	}
